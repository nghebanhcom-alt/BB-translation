...
server, so `AuthenticationError` is never raised here; connection failures
and non-2xx responses map to `TranslationProviderError`, and HTTP 429 (if the
local server is proxied/rate-limited) maps to `RateLimitError`.
"""
